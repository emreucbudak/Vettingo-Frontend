  ...
  return (
    <CandidateShell>
      <main aria-busy="true" aria-label="İş ilanları yükleniyor" className="mx-auto w-full max-w-[1440px] flex-1 px-4 py-8 md:px-8" role="status">
        ...
        <section className="overflow-hidden rounded border border-[#c5c6cd] bg-white"><div className="border-b border-[#c5c6cd] p-5"><Skeleton className="h-5 w-44" /></div><div className="divide-y divide-[#c5c6cd]">{Array.from({ length: 5 }, (_, index) => <div className="p-5" key={index}><Skeleton className="h-5 w-56 max-w-full" /><Skeleton className="mt-3 h-4 w-4/5" /><div className="mt-4 flex gap-2"><Skeleton className="h-7 w-20 rounded-full" /><Skeleton className="h-7 w-24 rounded-full" /></div></div>)}</div></section>
